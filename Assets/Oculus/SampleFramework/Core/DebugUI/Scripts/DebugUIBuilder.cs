...
public class DebugUIBuilder : MonoBehaviour
{
  ...
  private void RemoveRect(int index, int targetCanvas)
  {
      if (index >= insertedElements[targetCanvas].Count)
            return;
      Destroy(insertedElements[targetCanvas][index].gameObject);
      insertedElements[targetCanvas].RemoveAt(index);
      if (gameObject.activeInHierarchy)
      {
          Relayout();
      }
  ...
  public int rectNum(int targetCanvas = 0)
  {
        return insertedElements[targetCanvas].Count;
  }
  ...
  public void RemoveLabel(int index, int targetCanvas = 0)
  {
    RemoveRect(index, targetCanvas);
  } 
