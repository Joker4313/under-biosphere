	...
	void SnapTurn()
	{
			...
			(RotationEitherThumbstick && OVRInput.Get(OVRInput.Button.PrimaryThumbstickLeft)))
		{
			if (ReadyToSnapTurn)
			{
				...
				transform.RotateAround(CameraRig.centerEyeAnchor.position, Vector3.up, -RotationAngle * Time.fixedDeltaTime);
				FollowingUI.GetComponent<Following>().angle += -RotationAngle * Time.fixedDeltaTime;
			}
		}
